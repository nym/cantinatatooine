...
export function useDecoratedPerson(uri:string) {
    ...
    let { data: people, error: personError, isLoading: isPersonLoading } = useGetPersonListQuery({})
    // load all films because it's a relatively small dataset and will be cached
    const { data: films, error: filmsError, isLoading: isFilmsLoading } = useGetFilmListQuery({})
    // load all planets because it's a relatively small dataset and will be cached
    const { data: planets, error: planetsError, isLoading: isPlanetsLoading } = useGetPlanetListQuery({});
